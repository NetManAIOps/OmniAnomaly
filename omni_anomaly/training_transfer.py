...
class Trainer(VarScopeObject):
    ...
    Args:
        model (Omni): The :class:`Omni` model instance.
    ...
    def __init__(self, model, model_vs=None, n_z=None,
                 ...
                 name=None, scope=None, untrainable_variables_keyvalues=None):
        ...
        with reopen_variable_scope(self.variable_scope):
            ...
            if untrainable_variables_keyvalues is not None:
                ...
                all_name_list = [raw_name_list[v].name for v in range(len(raw_name_list))]
                untrainable_variables_list = []
                for kv in untrainable_variables_keyvalues:
                    untrainable_variables_list.extend([tfv for tfv in all_name_list if kv in tfv])
                convert_untrainable_variables_list = get_variable_via_scope(untrainable_variables_list)
            else:
    ...
    def model(self):
        """
        Get the :class:`Omni` model instance.

        Returns:
            Omni: The :class:`Omni` model instance.
        """
        return self._model

    def fit(self, values_list, sample_ratio=1.0,
        ...
        for values in values_list:
            ...
            v_x_list.append(v_x)
            train_excludes_list.append(np.hstack([[1], np.zeros([len(train_values)-1])]))
            v_excludes_list.append(np.hstack([[1], np.zeros([len(v_x)-1])]))
        train_values, v_x = np.vstack(train_values_list), np.vstack(v_x_list)
        train_excludes, v_excludes = np.hstack(train_excludes_list), np.hstack(v_excludes_list)
